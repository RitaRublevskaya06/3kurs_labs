...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        hostent* hst = gethostbyname("server.local");
        if (!hst)
            throw SetErrorMsgText("gethostbyname:", WSAGetLastError());

        serv.sin_addr.s_addr = *((u_long*)hst->h_addr);
        ...
        while (true)
        {
            ...
            scanf_s("%d", &service);
            char outMessage[16];
            strcpy_s(outMessage, sizeof(outMessage), get_message(service));
            ...
            if (service == 1)
            {
                ...
                {
                    Sleep(1000);
                    sprintf_s(outMessage, sizeof(outMessage), "%d", j);
                    if ((lobuf = send(cC, outMessage, strlen(outMessage) + 1, NULL)) == SOCKET_ERROR)
                        throw SetErrorMsgText("send:", WSAGetLastError());
                    printf("send: %s\n", outMessage);
                    if ((libuf = recv(cC, message, sizeof(message), NULL)) == SOCKET_ERROR)

...
int main()
{
    ...
    try
    {
        ...
        cout << "\n=== Демонстрация потери пакетов ===" << endl;
        cout << "Будет отправлено " << messageCount << " сообщений" << endl;
        ...
        {
            char outBuffer[50] = "Отправка пакетов от Client";
            char numberStr[10];
            itoa(i + 1, numberStr, 10);
            strcat(outBuffer, numberStr);

            if ((sendResult = sendto(clientSocket, outBuffer, strlen(outBuffer) + 1, NULL,
                (sockaddr*)&serverAddr, sizeof(serverAddr))) == SOCKET_ERROR)
                throw SetErrorMsgText("sendto:", WSAGetLastError());

            cout << "отправлено_" << (i + 1) << endl;
            Sleep(10);
        }

        int endTime = clock();
        int totalTime = endTime - startTime;
        cout << "\nПрограмма выполнялась " << totalTime << " тактов или "
            << ((float)totalTime) / CLOCKS_PER_SEC << " секунд.\n";

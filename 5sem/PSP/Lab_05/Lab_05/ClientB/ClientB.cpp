...
void main()
{
	...
	try
	{
		...
		GetServerByName("lenovo-home", "Hello", &from, &length);
...
bool GetServerByName(const char* name, const char* call, struct sockaddr* from, int* flen)
{
	...
	int buf = 0;
	int flen_size = sizeof(server);
	if ((buf = recvfrom(sS, message, sizeof(message), NULL, (sockaddr*)&server, &flen_size)) == SOCKET_ERROR)

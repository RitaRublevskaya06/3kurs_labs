...
int main(int argc, char* argv[])
{
	...
	try
	{
		...
		while (WaitForSingleObject(hStopEvent, 0) == WAIT_TIMEOUT)
		{
			...
			Element* el;
			Element* el1 = OS14::HTElement::CreateElementGet(h1, key.c_str(), key.length() + 1);
			Element* el2;
			...
			else
			{
				...
				el2 = OS14::HTElement::CreateElementInsert(h1, key.c_str(), key.length() + 1, payload.c_str(), payload.length() + 1);

				if (!OS14::DataManipulator::Update(h1, HT, el1, el2->payload, el2->payloadLength))
				{
					std::cout << OS14::Utils::getLastError(h1, HT) << std::endl;
				}
				else
				...
				if ((el = OS14::DataManipulator::Get(h1, HT, el2)) == NULL)
				{
					std::cout << OS14::Utils::getLastError(h1, HT) << std::endl;
				}
				delete el2;

// OS13_LIB.cpp : Defines the functions for the static library.
//
...
namespace OS13
{
	...
	namespace Manipulator
	{
		HTHANDLE* Create(OS13HANDEL h, int capacity, int secSnapshotInterval, int maxKeyLength, int maxPayloadLength, const wchar_t* fileName)
		{
			...
			{
				HTHANDLE* ht;
				HRESULT hr1 = pIManipulator->Create(&ht, capacity, secSnapshotInterval, maxKeyLength, maxPayloadLength, fileName);
				if (!SUCCEEDED(hr1))
				{
		...
		HTHANDLE* Open(OS13HANDEL h, const wchar_t* fileName, bool isMapFile)
		{
			IHTManipulator* pIManipulator = nullptr;
			HRESULT hr0 = ((IUnknown*)h)->QueryInterface(IID_IHTManipulator, (void**)&pIManipulator);

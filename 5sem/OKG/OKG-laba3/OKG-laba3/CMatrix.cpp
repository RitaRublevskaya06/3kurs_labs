...
// j - номер столбца
{
	if ((i > n_rows - 1) || (j > n_cols - 1))     //  проверка выхода за диапазон
	{
		...
		MessageBoxW(NULL, error, _T("Ошибка"), MB_ICONSTOP);
		exit(1);
	}
	return array[i][j];
}
...
// i - номер строки для вектора
{
	...
	{
		TCHAR* error = L"CMatrix::operator(int): объект не вектор - число столбцов больше 1 ";
		MessageBoxW(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	if (i > n_rows - 1)     //  проверка выхода за диапазон
	{
		TCHAR* error = TEXT("CMatrix::operator(int): выход индекса за границу диапазона ");
		MessageBoxW(NULL, error, TEXT("Ошибка"), MB_ICONSTOP);
		exit(1);
	}
	return array[i][0];
}
...
// Оператор M1+M2
{
	...
	if (!bb)
	{
		TCHAR* error = L"CMatrix::operator(+): несоответствие размерностей матриц ";
		MessageBoxW(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	CMatrix Temp(*this);
...
// Возвращает строку матрицы по номеру k
{
	if (k > n_rows - 1)
	{
		TCHAR* error = L"CMatrix::GetRow(int k): параметр k превышает число строк ";
		MessageBoxW(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	CMatrix M(1, n_cols);
...
// m - номер последнего элемента в строке
{
	...
	if (!b4)
	{
		TCHAR* error = L"CMatrix::GetRow(int k,int n, int m):ошибка в параметрах ";
		MessageBoxW(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	int nCols = m - n + 1;
...
// Возвращает столбец матрицы по номеру k
{
	if (k > n_cols - 1)
	{
		TCHAR* error = L"CMatrix::GetCol(int k): параметр k превышает число столбцов ";
		MessageBoxW(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	CMatrix M(n_rows, 1);
...
// m - номер последнего элемента в столбце
{
	...
	if (!b4)
	{
		TCHAR* error = L"CMatrix::GetCol(int k,int n, int m):ошибка в параметрах ";
		MessageBoxW(NULL, error, L"Ошибка", MB_ICONSTOP);
		exit(1);
	}
	int nRows = m - n + 1;
